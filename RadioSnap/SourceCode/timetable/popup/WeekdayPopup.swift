...
//  RadioSnap
//
//  Created by Mitsuhiro Shirai on 2025/05/29.
//
...
class WeekdayPopup {
    ...
    private static var retainedInstance: WeekdayPopup?

    static func show(from viewController: UIViewController, selectedIndex: Int, onSelect: @escaping (Int, String) -> Void) {
        let instance = WeekdayPopup()
        //Com.XLOG("初期設定曜日: selectedIndex=\(selectedIndex)）")
        retainedInstance = instance // 強参照で保持
        instance.present(from: viewController, selectedIndex: selectedIndex) { index, day in
            onSelect(index, day)
    ...
    private func present(from viewController: UIViewController, selectedIndex: Int, onSelect: @escaping (Int, String) -> Void) {
        let weekdays = ["全ての曜日", "日曜日", "月曜日", "火曜日", "水曜日", "木曜日", "金曜日", "土曜日"]
